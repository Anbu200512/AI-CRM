 ...
 * - Lines starting with ✓ → green check items
 * - Lines starting with • → styled bullets
 * - Lines starting with 🎯 📋 ✅ 🔍 📅 🔬 💊 📌 → section headers (bold)
 * - Lines starting with a number + . → numbered list items
 * - Empty lines → spacers
 */
  ...
  return (
    <div className="space-y-0.5 text-sm leading-relaxed">
      {lines.map((line, i) => {
        ...
        // Numbered list items (e.g., "1. Dr. Ravi – Apollo")
        if (/^\d+\./.test(line)) {
        ...
        // Indented sub-lines (4 spaces or tab)
        if (line.startsWith('   ') || line.startsWith('\t')) {
          return (
